...
def index():
    ...
    data = [{"id": row[0], "num": row[1], "created_at": datetime.strftime(row[2],'%Y/%m/%d %H:%M:%S')} for row in result]
    return jsonify(data)
...
def track():
   ...
   data = [{"id": result[0], "start": datetime.strftime(result[1],'%Y/%m/%d %H:%M:%S'), "end": datetime.strftime(result[1],'%Y/%m/%d %H:%M:%S')}]
...
def track_all():
    ...
    data = [{"id": result[0], "start": datetime.strftime(result[1],'%Y/%m/%d %H:%M:%S'), "end": datetime.strftime(result[2],'%Y/%m/%d %H:%M:%S')} for result in row]
...
def log():
    ...
    row = cursor.fetchall()
    data = [{"id":result[0], "Datetime":datetime.strftime(result[1],'%Y/%m/%d %H:%M:%S'),
...
def radiation():
    ...
    cursor.execute(sql)
    result = cursor.fetchall()
    data = [{"id": row[0], "figure": row[1], "created_at":datetime.strftime(row[2],'%Y/%m/%d %H:%M:%S')} for row in result]
...
def tracklog():
    ...
    # 트랙의 시작 및 종료 시간을 기반으로 레코드를 가져옵니다.
        ...
        cursor = db.cursor()
        cursor.execute(
        "SELECT * FROM record WHERE TrackId = %s", (track_id)
        )
        row = cursor.fetchall()
        data = {"results":[]}
        for i in range(len(row)):
